...
```prompt
Create a web3-focused authentication module for a NestJS application with:

1. Primary authentication strategy:
   - Web3/crypto wallet authentication (Ethereum, Polygon, BSC compatible)
   - JWT-based session management after wallet verification

2. Features:
   - User registration via wallet connection
   - Login with crypto wallet signature
   - Optional email association with verification
...
1. User entity with:
   ...
   - Email (optional, with verification status)
   - Username (required)
   - Profile information
...
4. User search and filtering by wallet address and optional parameters
...
2. Features:
   ...
   - Hot wallet management for token distribution
...
1. Entities for:
   ...
   - User progress
   - Achievements
   - Rewards (including token-based rewards)
...
5. Integration with blockchain for token rewards
6. Complete test coverage
```
...
1. Authentication integration with:
   - Web3 wallet-based authentication (primary)
