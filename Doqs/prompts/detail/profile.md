...
interface Profile {
  ...
  email?: string;                // Email is optional in our web3-focused system
  emailVerified?: boolean;       // Whether email has been verified, if provided
  firstName?: string;
  lastName?: string;
  displayName?: string;
  avatarUrl?: string;
  ...
  preferredNetwork?: string;     // Preferred blockchain network (e.g., 'ethereum', 'polygon')
  tokenBalanceVisibility?: VisibilityLevel; // Default: PRIVATE
  nftGalleryVisibility?: VisibilityLevel;   // Default: FRIENDS
...
export class ProfileService {
  ...
  async create(userId: string, profileData: Partial<Profile>): Promise<Profile> {
    ...
    // Get wallet address from auth user record
    const walletAddress = user.walletAddress;
    if (!walletAddress) {
      throw new BadRequestException('User does not have a valid wallet address');
    }
    ...
    if (profileData.email) {
      ...
      if (!this.validateEmailFormat(profileData.email)) {
        throw new BadRequestException('Invalid email format');
      }
      
      // Check if email is already in use
      const emailExists = await this.emailExists(profileData.email);
      if (emailExists) {
    ...
    // Set default blockchain network if not provided
    if (!profileData.preferredNetwork) {
      profileData.preferredNetwork = 'ethereum';
    }
  ...
  /**
   * Check if email exists in any profile
   */
  ...
  async sendEmailVerification(userId: string): Promise<void> {
    ...
    if (!profile.email) {
      throw new BadRequestException('Profile does not have an email to verify');
    }
    ...
    // Store token with expiration (implementation depends on your token storage)
    await this.tokenService.storeEmailVerificationToken(userId, token);
    
    // Send verification email
    await this.emailService.sendVerificationEmail(profile.email, token);
  }
  ...
  async verifyEmail(userId: string, token: string): Promise<void> {
    ...
    const isValid = await this.tokenService.validateEmailVerificationToken(userId, token);
    ...
    await this.tokenService.deleteEmailVerificationToken(userId);
  }
  ...
  private generateVerificationToken(): string {
    // Generate a random token (implementation depends on your security requirements)
...
async function handleAuthSuccess(walletAuthResult) {
  ...
  const walletInfo = {
    address: walletAuthResult.walletAddress,
    chainId: walletAuthResult.chainId,
...
function ProfileCreationForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    preferredNetwork: chainId ? getNetworkNameFromChainId(chainId) : 'ethereum',
    // ... other fields
  });
  ...
  async function handleSubmit(e) {
    ...
    try {
      // Display wallet address in the form
      ...
      await profileService.createProfile(formDataWithWallet);
      
      // If email was provided, offer to verify it
      if (formData.email) {
        await profileService.sendEmailVerification();
        toast.success(t('profile.emailVerification.sent'));
      }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {/* Wallet address display (non-editable) */}
      <div className="form-group">
        <label>{t('profile.labels.walletAddress')}</label>
        <div className="wallet-address">
          {shortenAddress(walletAddress)}
          <span className="network-badge">{getNetworkNameFromChainId(chainId)}</span>
        </div>
      </div>
      ...
      <div className="form-group">
        <label>{t('profile.labels.email')} <span className="optional">{t('common.optional')}</span></label>
        <input
          type="email"
        ...
        <small className="hint">{t('profile.emailOptionalHint')}</small>
      </div>
      ...
      <div className="form-group">
        <label>{t('profile.labels.preferredNetwork')}</label>
        <select
          value={formData.preferredNetwork}
          onChange={(e) => setFormData({...formData, preferredNetwork: e.target.value})}
        >
      ...
      <div className="form-group">
        <label>{t('profile.labels.tokenBalanceVisibility')}</label>
        <select
          value={formData.tokenBalanceVisibility || 'private'}
          onChange={(e) => setFormData({...formData, tokenBalanceVisibility: e.target.value})}
        >
          <option value="public">{t('profile.visibility.public')}</option>
          <option value="friends">{t('profile.visibility.friends')}</option>
          <option value="private">{t('profile.visibility.private')}</option>
        </select>
      </div>
      
      <div className="form-group">
        <label>{t('profile.labels.nftGalleryVisibility')}</label>
        <select
          value={formData.nftGalleryVisibility || 'friends'}
          onChange={(e) => setFormData({...formData, nftGalleryVisibility: e.target.value})}
        >
          <option value="public">{t('profile.visibility.public')}</option>
          <option value="friends">{t('profile.visibility.friends')}</option>
          <option value="private">{t('profile.visibility.private')}</option>
...
CREATE TABLE "profiles" (
  ...
  "wallet_address" VARCHAR(255) NOT NULL,  -- User's connected wallet address
  ...
  -- Web3 & Blockchain Settings
  "preferred_network" VARCHAR(50) DEFAULT 'ethereum',
  "token_balance_visibility" VARCHAR(20) DEFAULT 'private',
  "nft_gallery_visibility" VARCHAR(20) DEFAULT 'friends',
